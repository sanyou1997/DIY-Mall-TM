<script>
    ...
    // #ifdef MP-ALIPAY
    import { storeTmallLaunchParams } from './common/js/tmall-helper.js';
    // #endif
    export default {
        globalData: {
            data: {
                // 基础配置
                // 数据接口请求地址
                // 淘宝/支付宝小程序真机环境强制要求 HTTPS 域名，不允许 HTTP 或裸 IP
                request_url:'https://ostone.store/',

                // 静态资源地址（如系统根目录不在public目录下面请在静态地址后面加public目录、如：https://d1.shopxo.vip/public/）
                static_url:'https://ostone.store/',

                // 系统类型（默认default、如额外独立小程序、可与程序分身插件实现不同主体小程序及支付独立）
                system_type: 'default',

                // 基础信息
                application_title: '一生一石珠宝商城',
                application_describe: '天然晶石DIY',
            ...
            user_auth_login(object, method, params, auth_data) {
                console.log('user_auth_login 被调用');
                console.log('auth_data:', auth_data);
                var self = this;
                // #ifdef MP-JD
                // 京东小程序直接检查缓存，不使用 checkSession（可能不支持）
                console.log('京东小程序，直接检查缓存');
                var login_data = self.get_login_cache_info();
                console.log('缓存的 login_data:', login_data);
                if (login_data == null) {
                    console.log('login_data 为空，调用 user_login_with_auth');
                    self.user_login_with_auth(object, method, params, auth_data);
                } else {
                    console.log('login_data 存在，调用 get_user_login_info');
                ...
                uni.checkSession({
                    success: function () {
                        ...
                        console.log('缓存的 login_data:', login_data);
                        if (login_data == null) {
                            console.log('login_data 为空，调用 user_login');
                            self.user_login(object, method, params);
                        } else {
                            console.log('login_data 存在，调用 get_user_login_info');
                            self.get_user_login_info(object, method, params, login_data, auth_data);
                    ...
                    fail: function () {
                        console.log('checkSession 失败，重新登录');
             ...
             * auth_data  授权数据（用户信息）
             */
            user_login_with_auth(object, method, params, auth_data) {
                var self = this;
                uni.showLoading({
                    title: i18n.t('common.auth_in_text'),
                });
                // 京东小程序：使用 auth_data 中的 code 进行登录
                var code = auth_data ? auth_data.code : null;
                console.log('user_login_with_auth, code:', code);
                if (!code) {
                    uni.hideLoading();
                ...
                uni.request({
                    ...
                    success: (res) => {
                        uni.hideLoading();
                        console.log('user_login_with_auth 后端返回:', res.data);
                        if (res.data.code == 0) {
                            var data = res.data.data;
                            // 保存登录态
                            uni.setStorageSync(self.data.cache_user_login_key, data);
                            console.log('登录态已保存，继续获取用户信息');
                            // 继续调用 get_user_login_info 来更新用户信息
                            self.get_user_login_info(object, method, params, data, auth_data);
                        } else {
                            self.showToast(res.data.msg);
                        }
                    },
            ...
            /**
             * 使用 authCode 登录 - 淘宝/支付宝小程序专用
             * object     回调操作对象
             * method     回调操作对象的函数
             * params     回调操请求参数
             * authCode   授权码
             */
            user_login_with_authcode(object, method, params, authCode) {
                console.log('[App] user_login_with_authcode 被调用, authCode:', authCode);
                var self = this;
                uni.showLoading({
                    title: i18n.t('common.auth_in_text'),
                });
                ...
                const handleSuccess = (res) => {
                    console.log('[App] user_login_with_authcode 后端返回:', res.data || res);
                    uni.hideLoading();
                    // 兼容云应用返回格式
                    const responseData = res.data || res;
                    self.handle_login_response({ data: responseData }, self, object, method, params, true);
                };

                const handleFail = (err) => {
                    console.error('[App] user_login_with_authcode 请求失败:', err);
                    uni.hideLoading();
                    self.showToast(i18n.t('common.internet_error_tips'));
                };
                ...
                // #ifdef MP-ALIPAY
                // 淘宝小程序：使用云应用调用
                if (self.cloud && self.cloud.application) {
                    console.log('[App] 使用云应用调用登录接口');
                    self.cloud.application.httpRequest({
                        path: '/api.php',
                        method: 'POST',
                        headers: {
                        ...
                        console.log('[App] 云应用登录返回:', result);
                        handleSuccess({ data: result });
                    }).catch((err) => {
                        console.error('[App] 云应用登录失败:', err);
                        handleFail(err);
                    });
                    return;
                }
                ...
                const requestUrl = self.get_request_url('appminiuserauth', 'user');
                console.log('[App] 请求URL:', requestUrl);
                uni.request({
                    url: requestUrl,
                    method: 'POST',
                    data: {
            ...
            /**
             * 淘宝小程序云应用登录
             * 使用 cloud.application.httpRequest 调用后端，云网关会自动添加 open_id 等参数
             */
            taobao_cloud_login(object, method, params, auth_data) {
                console.log('[App] taobao_cloud_login 被调用');
                var self = this;
                uni.showLoading({
                    title: i18n.t('common.auth_in_text'),
                });

                const userInfo = auth_data.userInfo || {};
                const loginData = {
                    nickName: userInfo.nickName || auth_data.nickName || '',
                    avatar: userInfo.avatar || auth_data.avatar || '',
                    platform: 'taobao'
                };
                console.log('[App] 登录数据:', loginData);

                const handleSuccess = (result) => {
                    console.log('[App] 云应用登录返回:', JSON.stringify(result).substring(0, 300));
                    uni.hideLoading();
                    // 云网关可能已解包 { code:0, data:{...} } 为直接的用户数据
                    let data = result;
                    if (data && data.code === 0 && data.data) {
                        data = data.data;
                    }
                    // 判断登录是否成功：有 token 或 id 表示成功
                    const saveAndCallback = (userData) => {
                        uni.setStorageSync(self.data.cache_user_info_key, userData);
                        if (userData.token) {
                            self.token = userData.token;
                            self.data.token = userData.token;
                            // 确保 globalData.token 也能被 getToken() 读到
                    ...
                    if (data && (data.token || data.id || data.user_id)) {
                        saveAndCallback(data);
                    } else if (data && data.code === 0) {
                        saveAndCallback(data.data || data);
                    } else {
                        console.error('[App] 登录响应无法识别:', data);
                        self.showToast((data && data.msg) || '登录失败');
                    }
                };
                ...
                // 从多个位置尝试获取云 SDK
                const app = getApp();
                // 优先从 globalData 获取（因为 globalData 是 taobao_cloud_login 所在的对象）
                let cloud = self.cloud || (app && app.cloud) || (app && app.$vm && app.$vm.cloud) || (app && app.globalData && app.globalData.cloud);
                console.log('[App] 云 SDK 检查: self.cloud=', !!self.cloud, 'app.cloud=', !!(app && app.cloud), 'globalData.cloud=', !!(app && app.globalData && app.globalData.cloud));
                console.log('[App] cloud=', !!cloud, 'application=', !!(cloud && cloud.application));

                // 检查云 SDK 是否可用
                if (cloud && cloud.application) {
                    console.log('[App] 使用云应用 httpRequest');
                    const bodyParts = [];
                    Object.keys(loginData).forEach(k => {
                        if (loginData[k] !== undefined && loginData[k] !== null) {
                            bodyParts.push(encodeURIComponent(k) + '=' + encodeURIComponent(loginData[k]));
                        }
                    });
                    cloud.application.httpRequest({
                        ...
                        params: {
                            s: 'user/appMiniUserAuth',
                            ajax: 'ajax',
                            application_client_type: 'taobao'
                        },
                        body: bodyParts.join('&'),
                        exts: {
                            cloudAppId: '59508',
                    ...
                    // 云 SDK 不可用，尝试直接请求（可能会失败，但保留作为降级方案）
                    console.log('[App] 云 SDK 不可用，尝试直接请求');
                    const requestUrl = self.get_request_url('taobaouserauth', 'user');
                    console.log('[App] 请求URL:', requestUrl);
                    uni.request({
                        url: requestUrl,
                        method: 'POST',
            ...
            user_login_handle(object, method, params, is_to_auth = true) {
				...
				// #ifdef MP-JD
				// 京东小程序登录流程
				// 直接使用 jd.login 获取 code，不调用 jd.getUserInfo
				// jd.getUserInfo 需要用户点击按钮触发授权，直接调用会报错 [JdUserAuth]
				jd.login({
					success: (loginRes) => {
						if (loginRes.code) {
							// 将 code 发送到后端进行登录验证
							uni.request({
								...
								success: (res) => {
									uni.hideLoading();
									self.handle_login_response(res, self, object, method, params, is_to_auth);
								},
								fail: () => {
									uni.hideLoading();
									self.showToast(i18n.t('common.internet_error_tips'));
							...
							uni.hideLoading();
							self.showToast(i18n.t('login.login.3nmrg2')); // "登录失败"
						}
					},
					fail: (err) => {
						uni.hideLoading();
						console.error('jd.login fail:', err);
						self.showToast(i18n.t('login.login.3nmrg2'));
					},
				})
				// #endif
                ...
                action = 'getLoginCode';
                // #endif
                console.log('[App] user_login_handle: 调用 uni.' + action);
                uni[action]({
                    success: (res) => {
                        console.log('[App] uni.' + action + ' 成功:', JSON.stringify(res));
                        // 微信用 code，淘宝/支付宝用 authCode
                        const authcode = res.code || res.authCode;
                        console.log('[App] authcode:', authcode);
                        if (authcode) {
                            const loginUrl = self.get_request_url('appminiuserauth', 'user');
                            const loginData = { authcode: authcode };
                            const onSuccess = (res) => {
                                uni.hideLoading();
                                self.handle_login_response(res, self, object, method, params, is_to_auth);
                            };
                            ...
                            const cloud = self.cloud || self.globalData?.cloud;
                            if (cloud && cloud.application) {
                                const lParts = Object.keys(loginData).map(k => encodeURIComponent(k) + '=' + encodeURIComponent(loginData[k]));
                                cloud.application.httpRequest({
                                    path: '/api.php',
                                    method: 'POST',
                                    headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' },
                                    params: { s: 'user/appminiuserauth', ajax: 'ajax' },
                                    body: lParts.join('&'),
                                    exts: { cloudAppId: '59508', timeout: 30000 }
                                }).then((result) => {
                                    console.log('[App] 云应用登录返回:', result);
                                    uni.hideLoading();
                                    // 云网关可能已解包，重新包装
                                    let data = result;
                                    if (data && data.code === undefined && (data.token || data.id)) {
                                        data = { code: 0, data: data };
                                    }
                                    self.handle_login_response({ data: data }, self, object, method, params, is_to_auth);
                                }).catch((err) => {
                                    console.error('[App] 云应用登录失败:', err);
                                    onFail();
                                });
                    ...
                    fail: (e) => {
                        console.error('[App] uni.' + action + ' 失败:', JSON.stringify(e));
                        uni.hideLoading();
                        self.showToast(i18n.t('login.login.3nmrg2'));
                    },
                });
			...
			handle_login_response(res, self, object, method, params, is_to_auth) {
			    if (res.data.code == 0) {
			        var data = res.data.data;
            ...
            get_user_login_info(object, method, params, login_data, auth_data) {
                console.log('get_user_login_info 被调用');
                ...
                // #ifdef MP-JD
                // 京东小程序：如果 login_data 中没有 jd_openid，需要重新用 code 获取
                if (!login_data.jd_openid && !login_data.openid && auth_data && auth_data.code) {
                    console.log('京东小程序 openid 为空，使用 code 重新获取');
                    uni.showLoading({
                        title: i18n.t('common.auth_in_text'),
                    });
                    uni.request({
                        ...
                        method: 'POST',
                        data: {
                            authcode: auth_data.code,
                        },
                        dataType: 'json',
                        success: (res) => {
                            console.log('京东重新获取 openid 返回:', res.data);
                            console.log('京东重新获取 openid 返回 data 详情:', JSON.stringify(res.data.data));
                            if (res.data.code == 0) {
                                var newLoginData = res.data.data;
                                console.log('newLoginData.openid:', newLoginData.openid);
                                // 更新缓存
                                uni.setStorageSync(self.data.cache_user_login_key, newLoginData);
                                // 使用新的 login_data 继续请求
                                self._do_get_user_login_info(object, method, params, newLoginData, auth_data);
                            } else {
                                uni.hideLoading();
                                self.showToast(res.data.msg);
                            }
                ...
                self._do_get_user_login_info(object, method, params, login_data, auth_data);
            },

            /**
             * 实际执行获取用户授权信息的请求
             */
            _do_get_user_login_info(object, method, params, login_data, auth_data) {
                var self = this;

                // 根据平台获取对应的 openid 和 unionid
                var openid = login_data.openid;
                var unionid = login_data.unionid;
                // #ifdef MP-JD
                // 京东小程序使用 jd_openid 和 jd_unionid
                openid = login_data.jd_openid || login_data.openid;
                unionid = login_data.jd_unionid || login_data.unionid;
                // #endif
                ...
                uni.request({
                    ...
                    dataType: 'json',
                    success: (res) => {
                        console.log('appminiuserinfo 接口返回:', res.data);
                        uni.hideLoading();
                        if (res.data.code == 0) {
                            console.log('授权成功，保存用户信息');
                            uni.setStorageSync(self.data.cache_user_info_key, res.data.data);
                            if (typeof object === 'object' && (method || null) != null) {
                                console.log('调用回调方法:', method);
                                object[method](params);
                            } else {
                                console.log('没有回调方法');
                            }
                        } else {
                            console.error('授权失败:', res.data.msg);
                            self.showToast(res.data.msg);
                        }
                    },
                    fail: (err) => {
            ...
            page_share_handle(share = null) {
				// #ifdef MP-JD
				// 京东小程序分享处理，可暂时留空或使用其自有API
				console.log('京东小程序分享，功能待实现');
				return;
				// #endif
                // 当前页面
                var pages = getCurrentPages();
            ...
            init_config(num = 0, object, method, params) {
                var self = this;
                console.log('[App] init_config 开始, num=', num);
                if (num == 0) {
                    self.data.common_data_init_config_back_list.push({ object: object, method: method, params: params})
                }

                // #ifdef MP-ALIPAY
                // 淘宝/支付宝小程序：禁止配置域名白名单，直接使用离线模式
                console.log('[App] MP-ALIPAY: 跳过远程配置，使用离线模式');
                ...
                uni.getNetworkType({
                    success: function (res) {
                        console.log('[App] getNetworkType:', res.networkType);
                        if (res.networkType != 'none') {
                            // 获取配置本地缓存
                            ...
                            if(self.data.common_data_loading_status == 0) {
                                ...
                                uni.request({
                                    ...
                                    dataType: 'json',
                                    success: (res) => {
                                        console.log('[App] init_config 请求成功, code=', res.data?.code);
                                        // 赋值不在加载状态
                                    ...
                                    fail: (err) => {
                                        console.error('[App] init_config 请求失败:', err);
                                        // 赋值不在加载状态
                                        self.data.common_data_loading_status = 0;
                                        // 首次则再次初始化配置
            ...
            application_client_type() {
                ...
                try {
                    ...
                    // 检查多个可能的字段来判断是否为淘宝环境
                    const app = (sysInfo.app || '').toLowerCase();
                    const clientName = (sysInfo.clientName || '').toLowerCase();
                    const host = (sysInfo.host?.appId || sysInfo.hostPackageName || '').toLowerCase();
                    const appId = (sysInfo.appId || '').toLowerCase();
                    // 淘宝小程序标识：tb, taobao, tmall, 淘宝, 天猫
                    const taobaoKeywords = ['tb', 'taobao', 'tmall', '淘宝', '天猫'];
                    const checkFields = [app, clientName, host, appId];
                    let isTaobao = false;
                    for (const field of checkFields) {
                        for (const keyword of taobaoKeywords) {
                ...
                if(this.data.is_use_native_tabbar == 1) {
                    ...
                    for(var i in temp_system_tabbar) {
                        switch(temp_system_tabbar[i]) {
                            case '/pages/bracelet/bracelet' :
							    uni.setTabBarItem({
							        index: parseInt(i),
							        iconPath: 'static/images/common/tabbar/bracelet.png',
							        selectedIconPath: 'static/images/' + theme + '/tabbar/bracelet.png',
							        text: i18n.t('DIY定制'),
							    });
							    break;	
        ...
        onLaunch(params) {
            ...
            try {
                ...
                console.log('[App] 云 SDK 初始化成功');
            } catch (e) {
                console.error('[App] 云 SDK 初始化失败:', e);
            }
            ...
            if (params) {
                var jdParams = null;
                // 直接参数模式
                if (params.customInstanceId || params.custom_instance_id || params.skuId || params.sku_id) {
                    jdParams = {
                        customInstanceId: params.customInstanceId || params.custom_instance_id || '',
                        userId: params.userId || params.user_id || '',
                        skuId: params.skuId || params.sku_id || '',
                    };
                }
                // scene 参数模式 (京东可能通过 scene 字符串传参)
                if (!jdParams && params.query && params.query.scene) {
                    try {
                        var scene = decodeURIComponent(params.query.scene);
                        var sceneParams = this.globalData.url_params_to_json(scene);
                        if (sceneParams.customInstanceId) {
                            jdParams = {
                                customInstanceId: sceneParams.customInstanceId || '',
                                userId: sceneParams.userId || '',
                                skuId: sceneParams.skuId || '',
                            };
                        }
                    } catch (e) {
                        console.warn('[JD C2M] scene parse error:', e);
                    }
                }
                if (jdParams) {
                    this.globalData.jd_c2m_params = jdParams;
                    try {
                        uni.setStorageSync('jd_c2m_params', JSON.stringify(jdParams));
                    } catch (e) {}
                    console.log('[JD C2M] App onLaunch params:', jdParams);
                }
            }
            // #endif
        },
