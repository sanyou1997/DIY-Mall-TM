                ...
                <view class="flex-col jc-sb dom-content">
                    <view>
                        ...
                        <view v-if="current_opt_form == 'auth' && (user || null) == null" class="margin-top-xxxl tc">
                            ...
                            <!-- #ifdef MP-ALIPAY -->
                            <view class="margin-top-xl tc">
                                <text class="cr-grey-9 text-size-sm" @tap="alipay_phone_login_event">手机号验证码登录</text>
                            </view>
                            <!-- #endif -->
                        </view>
                        <!-- #endif -->
        ...
        methods: {
            ...
            init_config(status) {
                if ((status || false) == true) {
                    var verifyBindMobileList = app.globalData.get_config('config.common_user_verify_bind_mobile_list', []) || [];
                    var onekeyBindMobileList = app.globalData.get_config('config.common_user_onekey_bind_mobile_list', []) || [];

                    // 京东小程序不需要绑定手机号，已在 user_auth_back_event 中跳过

                    // #ifdef MP-ALIPAY
                    // 淘宝/支付宝小程序：使用默认 logo（无法从后端获取配置）
                    var siteLogo = '/static/logo/logo.png';
                    // #endif
                    // #ifndef MP-ALIPAY
                    var siteLogo = app.globalData.get_config('config.home_site_logo_square');
                    // #endif

                    this.setData({
                        home_site_logo_square: siteLogo,
                        common_user_verify_bind_mobile_list: verifyBindMobileList,
                        common_user_onekey_bind_mobile_list: onekeyBindMobileList,
            ...
            // #ifdef MP-ALIPAY
            // 淘宝小程序：切换到手机号验证码登录
            alipay_phone_login_event() {
                this.setData({
                    current_opt_form: 'login',
                    current_opt_type: 'login_sms',
                });
            },
            // #endif
            ...
            get_user_info_event(e) {
                console.log('[login] get_user_info_event 被调用, agreement_status:', this.agreement_status);
                // 是否已同意协议
                ...
                // #endif
                // #ifdef MP-JD
                // 京东小程序：jd.login 获取 code，后端换取 openid
                // JD SDK 没有 getUserInfo API，用户信息由后端通过 code 获取
                jd.login({
                    success: (loginRes) => {
                        if (loginRes.code) {
                            this.user_auth_code({ code: loginRes.code });
                        } else {
                            app.globalData.showToast('登录失败');
                        }
                    },
                    fail: (err) => {
                        app.globalData.showToast('登录失败：' + (err.errMsg || '未知错误'));
                    }
                });
                // #endif
            },
            ...
            // 支付宝/淘宝小程序授权失败回调
            alipay_auth_error(e) {
                console.error('[login] alipay_auth_error:', e);
                app.globalData.showToast('用户拒绝授权');
            },

            // 用户授权登录
            // auth_data  授权数据
            user_auth_code(auth_data) {
                console.log('user_auth_code 被调用，auth_data:', auth_data);
                if ((auth_data || null) != null) {
                    console.log('准备调用 user_auth_login');
                    app.globalData.user_auth_login(this, 'user_auth_back_event', null, auth_data);
                } else {
                    console.log('auth_data  为空');
            ...
            user_auth_back_event() {
                console.log('user_auth_back_event 被调用');
                var user = app.globalData.get_user_cache_info() || null;
                ...
                // #ifdef MP-JD
                // 京东小程序不需要绑定手机号，授权后直接返回
                if (user) {
                    console.log('京东小程序：跳过手机绑定，直接返回');
                    uni.navigateBack();
                    return;
                }
                // #endif
                ...
                // 是否需要绑定手机号码
                // user_is_bind_mobile 返回 true = 需要强制绑定, false = 不需要
                var needBindMobile = app.globalData.user_is_bind_mobile(user);
            ...
            formBind(e, validation, url) {
                ...
                // 优先从 login_data 获取 openid，因为 user 信息可能不包含 openid
                e.detail.value[field_openid] = user[field_openid] || login_data[field_openid] || login_data.openid || '';
                e.detail.value[field_unionid] = user[field_unionid] || login_data[field_unionid] || login_data.unionid || '';
                // #endif

                // #ifdef MP-JD
                // 京东小程序特殊处理：确保 jd_openid 有值
                if (!e.detail.value[field_openid] && login_data.openid) {
