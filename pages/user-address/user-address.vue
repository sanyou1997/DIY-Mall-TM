        ...
        <view class="bottom-fixed" :style="bottom_fixed_style">
            <view class="bottom-line-exclude">
                ...
                <view class="flex-row jc-sb align-c gap-10">
                    ...
                    <button class="item br-main cr-main bg-white round text-size wh-auto" type="default" hover-class="none" @tap="choose_system_address_event">导入京东地址</button>
                </view>
                <!-- #endif -->
                <!-- #ifndef MP-JD -->
    ...
    export default {
        ...
        methods: {
            ...
            choose_system_address_event(e) {
                ...
                // #ifdef MP-JD
                // 京东小程序使用 jd.chooseAddress 获取收货地址
                this.choose_jd_address();
                return;
                // #endif
            ...
            // #ifdef MP-JD
            // 京东小程序获取收货地址 - 处理地址数据并保存
            _handleJdAddressResult(res) {
                var self = this;
                console.log('[user-address] 京东获取收货地址成功:', JSON.stringify(res));
                // 京东返回的地址数据结构可能有多种字段名
                var data = {
                    name: res.userName || res.nickName || res.realName || res.name || '',
                    tel: res.telNumber || res.mobileNumber || res.phone || res.tel || '',
                    province: res.provinceName || res.province || '',
                    city: res.cityName || res.city || '',
                    county: res.countyName || res.districtName || res.county || res.district || '',
                    address: res.detailInfo || res.addressDetail || res.streetName || res.detail || res.address || '',
                };
                console.log('[user-address] 解析后的地址数据:', JSON.stringify(data));

                // 检查地址数据是否有效（至少有姓名和电话）
                if (data.name && data.tel) {
                    uni.showLoading({
                        title: self.$t('common.processing_in_text') || '处理中...',
                    });
                    // 调用后端接口保存地址
                    uni.request({
                        url: app.globalData.get_request_url('outsystemadd', 'useraddress'),
                        ...
                        success: (apiRes) => {
                            uni.hideLoading();
                            console.log('[user-address] 保存地址响应:', JSON.stringify(apiRes.data));
                            if (apiRes.data.code == 0) {
                                app.globalData.showToast('地址导入成功');
                                if (self.is_auto_import_system_address_choose == 1) {
                                    self.address_choose_cache_save_back(apiRes.data.data);
                                } else {
                                    self.get_data_list();
                                }
                            } else {
                                if (app.globalData.is_login_check(apiRes.data)) {
                                    app.globalData.showToast(apiRes.data.msg);
                                } else {
                                    app.globalData.showToast(self.$t('common.sub_error_retry_tips') || '提交失败，请重试');
                                }
                            }
                        },
                        fail: (apiErr) => {
                            uni.hideLoading();
                            console.error('[user-address] 保存地址请求失败:', apiErr);
                            app.globalData.showToast(self.$t('common.internet_error_tips') || '网络错误');
                        },
                    });
                } else {
                    console.warn('[user-address] 地址信息不完整:', data);
                    app.globalData.showToast('获取的地址信息不完整，请重试');
                }
            },

            // 京东小程序获取收货地址 - 处理错误
            _handleJdAddressError(err) {
                console.error('[user-address] 京东获取收货地址失败:', JSON.stringify(err));
                ...
                // 需要授权
                if (errMsg.indexOf('auth') !== -1 || errMsg.indexOf('authorize') !== -1 || errMsg.indexOf('permission') !== -1 || errMsg.indexOf('deny') !== -1) {
                    uni.showModal({
                        title: '授权提示',
                        content: '需要获取您的收货地址权限，请在设置中开启',
                        confirmText: '去设置',
                        cancelText: '取消',
                        success: (modalRes) => {
                            if (modalRes.confirm) {
                                uni.openSetting && uni.openSetting({
                                    success: (settingRes) => {
                                        console.log('[user-address] 设置页返回:', JSON.stringify(settingRes));
                                    }
                                });
                            }
                        }
                ...
                // 其他错误
                app.globalData.showToast('获取地址失败：' + errMsg);
            },

            // 京东小程序获取收货地址
            choose_jd_address() {
                var self = this;
                console.log('[user-address] choose_jd_address called');

                // 打印可用的 API
                console.log('[user-address] typeof jd:', typeof jd);
                console.log('[user-address] typeof uni.chooseAddress:', typeof uni.chooseAddress);
                if (typeof jd !== 'undefined') {
                    console.log('[user-address] jd.chooseAddress:', typeof jd.chooseAddress);
                    console.log('[user-address] jd.openAddress:', typeof jd.openAddress);
                    console.log('[user-address] jd.getAddress:', typeof jd.getAddress);
                }

                // 方法1: 尝试 jd.chooseAddress (京东原生 API)
                if (typeof jd !== 'undefined' && typeof jd.chooseAddress === 'function') {
                    console.log('[user-address] 尝试 jd.chooseAddress');
                    try {
                        jd.chooseAddress({
                            success: (res) => {
                                console.log('[user-address] jd.chooseAddress success callback');
                                self._handleJdAddressResult(res);
                            },
                            fail: (err) => {
                                console.log('[user-address] jd.chooseAddress fail callback');
                                self._handleJdAddressError(err);
                            },
                            complete: () => {
                                console.log('[user-address] jd.chooseAddress complete callback');
                            }
                        });
                        console.log('[user-address] jd.chooseAddress 调用完成，等待回调...');
                    } catch (e) {
                        console.error('[user-address] jd.chooseAddress 调用异常:', e);
                        app.globalData.showToast('调用地址选择器失败');
                    }
                    return;
                }

                // 方法2: 尝试 uni.chooseAddress (uni-app 统一封装)
                if (typeof uni.chooseAddress === 'function') {
                    console.log('[user-address] 尝试 uni.chooseAddress');
                    try {
                        uni.chooseAddress({
                            success: (res) => {
                                console.log('[user-address] uni.chooseAddress success callback');
                                self._handleJdAddressResult(res);
                            },
                            fail: (err) => {
                                console.log('[user-address] uni.chooseAddress fail callback');
                                self._handleJdAddressError(err);
                            },
                            complete: () => {
                                console.log('[user-address] uni.chooseAddress complete callback');
                            }
                        });
                        console.log('[user-address] uni.chooseAddress 调用完成，等待回调...');
                    } catch (e) {
                        console.error('[user-address] uni.chooseAddress 调用异常:', e);
                        app.globalData.showToast('调用地址选择器失败');
                    }
                    return;
                }
                ...
                // 都不可用
                console.error('[user-address] 没有可用的地址选择 API');
                app.globalData.showToast('当前环境不支持获取京东地址');
            },
            // #endif
