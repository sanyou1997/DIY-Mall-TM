                                    ...
                                    <view class="cart-goods-item flex-row align-c bs-bb padding-main pr wh-auto" :class="(common_site_type == 4 ? 'cart-exhibition-mode-data' : '')">
                                        ...
                                        <view class="items oh padding-left-main flex-1 flex-row">
                                            ...
                                            <view class="cart-goods-base padding-left-main flex-1">
                                                ...
                                                <view v-if="cart_spec_display(item).length > 0" class="margin-bottom-sm">
                                                    <block v-for="(sv, si) in cart_spec_display(item)" :key="si">
                                                        <text v-if="si > 0" class="cr-grey padding-left-xs padding-right-xs">;</text>
                                                        <text class="cr-grey">{{ sv }}</text>
                                                    </block>
                                                </view>
    ...
    export default {
        ...
        methods: {
            ...
            goods_event(e) {
                ...
                // 定制手串：若存在作品ID则跳转到作品详情
                const workId = this.get_cart_work_id(goods);
                if (workId) {
                    uni.navigateTo({ url: '/pages/bracelet-sub/work-detail?work_id=' + workId });
                    return;
                }
            ...
            // 从购物车商品中解析作品ID（兼容多来源字段）
            get_cart_work_id(goods) {
                if (!goods) return null;
                if (goods.work_id) return goods.work_id;
                if (goods.custom_meta && goods.custom_meta.work_id) return goods.custom_meta.work_id;
                    let workId = null;
                    if (Array.isArray(goods.custom_spec)) {
                        for (let i = 0; i < goods.custom_spec.length; i++) {
                            const cs = goods.custom_spec[i];
                            if (cs && cs.type === 'custom_work_id' && cs.value) {
                                workId = cs.value;
                                break;
                            }
                        }
                    }
                    if (!workId && Array.isArray(goods.spec)) {
                        for (let i = 0; i < goods.spec.length; i++) {
                            const cs = goods.spec[i];
                            if (cs && cs.type === 'custom_work_id' && cs.value) {
                                workId = cs.value;
                                break;
                            }
            ...
            // 购物车规格展示（包含定制明细摘要）
            cart_spec_display(item) {
                const list = [];
                if (item && Array.isArray(item.spec)) {
                    item.spec.forEach((sv) => {
                        const value = (sv && sv.value != null) ? String(sv.value).trim() : '';
                        if (value) list.push(value);
                    });
                }
                const parts = this.get_custom_design_parts(item);
                const summary = this.summarize_design_parts(parts);
                if (summary) list.push(summary);
                return list;
            },

            get_custom_design_parts(item) {
                if (!item) return [];
                if (Array.isArray(item.custom_design_parts)) return item.custom_design_parts;
                let raw = null;
                if (Array.isArray(item.custom_spec)) {
                    for (let i = 0; i < item.custom_spec.length; i++) {
                        const cs = item.custom_spec[i];
                        if (cs && cs.type === 'custom_design_parts' && cs.value != null) {
                            raw = cs.value;
                            break;
                        }
                    }
                }
                if (!raw && item.custom_design_parts_raw) {
                    raw = item.custom_design_parts_raw;
                }
                return this.parse_custom_design_parts(raw);
            },

            parse_custom_design_parts(raw) {
                if (!raw) return [];
            ...
            summarize_design_parts(parts) {
                if (!Array.isArray(parts) || parts.length === 0) return '';
                const map = {};
                parts.forEach((p) => {
                    if (!p) return;
                    const name = String(p.name || p.title || '').trim();
                    const size = String(p.size || p.spec || p.specification || '').trim();
                    if (!name && !size) return;
                    const key = `${name}|${size}`;
                    const qty = Number(p.qty || p.count || p.num || 1) || 1;
                    if (!map[key]) {
                        map[key] = { name, size, qty: 0 };
                    }
                    map[key].qty += qty;
                });
                const list = Object.values(map).map((item) => {
                    const sizeText = item.size ? `(${item.size})` : '';
                    return `${item.name}${sizeText}x${item.qty}`;
                });
                if (list.length === 0) return '';
                const max = 2;
                const head = list.slice(0, max);
                return list.length > max ? `定制：${head.join('，')} 等${list.length}种` : `定制：${head.join('，')}`;
            }
        }
    };
</script>
